...
Otherwise, install Python from the website [#]_.
...
Also see the ``math`` and ``random`` modules in
...
.. table:: Number magic methods
  ...
  ``long(num)``          ``__long__``       Long coercion
...
.. table:: Integer specific methods and operations 
  ...
  ``hex(num)``         ``__hex__``        Hex conversion
...
Python 2 strings hold byte strings. To create Unicode strings, you need to use a Unicode literal. Python has a few ways to represent strings:
...
.. table:: Escape Characters (Unicode escapes must be in a Unicode literal)
...
.. table:: String methods
  ...
  ``s.decode(encoding, errors= 'strict')``                Encode a (byte) string into a Unicode string
  ``s.encode(encoding, errors= 'strict')``                Encode a (Unicode) string into a byte string
...
.. table:: Set Methods
  ...
  ======================================= ========================= ============================================================
  Operation/Method                        Provided By               Result
  ======================================= ========================= ============================================================
...
.. table:: Built in callables
  ...
  ``intern(s)``                                                     Intern a string in global table
  ...
  ``next(iter, [default])``                                         Get next item from iteration protocol (call ``iter.next()``), if ``default`` provide return instead of raising ``StopIteration``
  ``object``                                                        Root base type
  ``oct(i)``                                                        String containing octal version of number (``int(oct(i), 8)`` to reverse)
  ``open(filename, [mode], [buffereing])``                          Open a file
  ...
  ``print val, [val2 ...][,]``                                      Print values to stdout. Print protocol (call ``val.__str__()``)
  ``@property``                                                     Decorator to turn a method into an attribute
  ``quit()``                                                        Quit interpreter
  ``range([start], stop, [step])``                                  Return list from ``start`` (default ``0``) to ``stop - 1``, by ``step`` increments (default ``1``)
  ``raw_input([prompt])``                                           Read input from user
  ``reduce(f, seq, [initial])``                                     Perform reduction by calling ``f`` on initial pair from ``seq``, then result with next value.
  ``reload(module)``                                                Reload module (doesn't swap out existing instances of classes from module)
  ...
  ``super(type, [obj])``                                            Get access to superclass
  ...
  ``unichr(num)``                                                   Return Unicode string from integer ``num``
  ``unicode(s)``                                                    Coerce ``s`` to a Unicode string
  ``xrange([start], stop, [step])``                                  Return range object that iterates from ``start`` (default ``0``) to ``stop - 1``, by ``step`` increments (default ``1``)
  ``zip(seq1, [seq2, ...])``                                        Return list of tuples of ``(seq1[0], seq2[0])``, ``(seq1[1], seq2[1])``, ... until shortest sequence
  ================================================================= ============================================================
...
Python 2 represents strings byte sequences. To create a literal Unicode string with Unicode
characters, we
need to put a ``u`` in front of the string. We can *encode* Unicode strings to a series of
bytes such as UTF-8. If we have a normal string (bytes), we can *decode* them to a Unicode string::
...
If we call ``print`` we will see the user friendly representation (calling ``__str__`` rather than ``__repr__``)::
...
.. table:: File Methods/Attributes
  ...
  ``f.next()``                                                      Return next item of iteration (line in text)
  ``f.softspace``                                                   Flag if space needs to be printed
  ``f.read( size=-1)``                                              Read ``size`` characters (``-1`` is whole file)
  ``f.readinto()``                                                  Mystery method
...
Functions may take input, do some processing, and may return output. You may
...
.. note::
  ...
  We use whitespace to specify a block in Python. We typically indent following a colon. PEP 8 recommends using 4 spaces.

We can create anonymous functions using the ``lambda`` statement. Because this only allows expressions, it is somewhat crippled in functionality::
...
.. note::
  ...
  The ``super`` function will not work with old-style classes. Make sure
  your base classes inherit from ``object`` in Python 2.
...
To make an iterator implement ``__iter__`` and  ``next``::
...
You can define the ``__nonzero__`` method to teach your classes how to act in a boolean context. If that doesn't exists, Python will use ``__len__``, and finally default to ``True``.
...
Python 2 does not include the ability to create *virtual environments* out of the box. To install the ``virtualenv`` module for creating a sandbox for your project, run (you might need to run with ``sudo``)::

  $ pip install virtualenv

To create an environment on Unix and Windows systems, run::
